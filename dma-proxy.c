...
static long ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if(ret != 0)
	{
		printk(KERN_ERR "%s: ioctl(), can't transfer DMA buffer\n", pchannel_p->name);
		return -EPERM;
	}

	if(value < 0)
	{
		printk(KERN_ERR "%s: invalid ioctl argument\n", pchannel_p->name);
		return -EINVAL;
	} 
...
static int create_channel(struct platform_device *pdev, struct dma_proxy_channel *pchannel_p, char *name, u32 direction)
{
	...
	printk(KERN_INFO "Allocating memory, virtual address: 0x%016X physical address: 0x%016X\n", 
			(unsigned int)pchannel_p->buffer_table_p, (unsigned int)(&pchannel_p->buffer_phys_addr));
